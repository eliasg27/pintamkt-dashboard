import{useEffect,useState,useRef}from'react';import{useRouter}from'next/router';import Head from'next/head';import ClientDashboard from'../components/ClientDashboard';import{useAuth}from'../context/AuthContext';import{sb}from'../lib/supabase';
...
const SIDEBAR_CHANNELS=[
  {key:'meta',       label:'Meta Ads',        icon:'icono_meta_ads.png'},
  {key:'instagram',  label:'Instagram',        icon:'icono_instagram.svg'},
  {key:'facebook',   label:'Facebook',         icon:'icono_facebook.svg'},
  {key:'ga4',        label:'Google Analytics', icon:'google_analitycs.svg'},
  {key:'googleads',  label:'Google Ads',       icon:'icono_google_ads.svg'},
  {key:'woocommerce',label:'Ecommerce',        icon:'ecommerce.svg'},
  {key:'linkedin',   label:'LinkedIn',         icon:'linkedin.png'},
  {key:'bot',        label:'Bot',              icon:'bot.svg'},
];
...
function openClient(c){setCurrent(c);setPage('client');setActiveCh(null);}
function openClientAndConfig(c){setCurrent(c);setPage('client');setActiveCh(null);setEditMods({...DEFAULT_MODS,...(c.modulos||{})});setShowConfig(true);}
...
function OverviewSpark({data,color}){
  ...
  const n=data.length;
  const points=data.map((v,i)=>`${(i/(n-1))*100},${10-((v-min)/range)*9}`).join(' ');
  ...
  const hx=hov!=null?(hov/(n-1))*100:0;
  const hy=hov!=null?10-((data[hov]-min)/range)*9:0;
  const tx=hov!=null?Math.min(Math.max(hx,18),82):0;
  return(
    <div style={{position:'relative',height:22,width:'60%',marginTop:2,opacity:.72,cursor:'crosshair'}} onMouseMove={onMove} onMouseLeave={()=>setHov(null)}>
      <svg viewBox="0 0 100 12" preserveAspectRatio="none" style={{height:'100%',width:'100%',display:'block'}}>
        <polyline points={points} fill="none" stroke={color} strokeWidth=".7" strokeLinecap="round" strokeLinejoin="round"/>
        {hov!=null&&<>
          <line x1={hx} y1="0" x2={hx} y2="11" stroke={color} strokeWidth=".5" strokeDasharray="1.5,1.5" opacity=".55"/>
          <circle cx={hx} cy={hy} r="1.4" fill={color} stroke="#fff" strokeWidth=".7"/>
        </>}
      </svg>
      {hov!=null&&<div style={{position:'absolute',left:`${tx}%`,top:-30,transform:'translateX(-50%)',background:'rgba(20,20,18,.9)',color:'#fff',borderRadius:5,padding:'4px 6px',fontSize:10,fontWeight:700,lineHeight:1,pointerEvents:'none',boxShadow:'0 4px 12px rgba(0,0,0,.18)',zIndex:3,whiteSpace:'nowrap'}}>
        <span style={{display:'inline-block',width:7,height:7,borderRadius:2,background:color,marginRight:4,boxShadow:'0 0 0 1px rgba(255,255,255,.85)'}}/>
        {data[hov]}
      </div>}
    </div>
  );
...
<div className="lay">
  <div className={`sb${mobileNavOpen?' mobile-open':''}`}>
    <div className="sb-hd">
    ...
    <div className="sb-nav">
      ...
      <div className="dv"/>
      <div className="ss">Canales</div>
      {SIDEBAR_CHANNELS.map(ch=>{
        const isActive=activeCh===ch.key&&page==='client';
        const enabled=page==='client';
        return(
          <div key={ch.key}
            className={`ni${isActive?' ac':''}`}
            onClick={()=>{if(enabled)setActiveCh(activeCh===ch.key?null:ch.key);}}
            style={{opacity:enabled?1:0.35,cursor:enabled?'pointer':'default',borderRightColor:isActive?'#EBE300':'transparent'}}>
      ...
      <div className="ni" onClick={()=>{setEditNombre(profile?.nombre||'Equipo Pinta');setShowSettings(true);}}>
        <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{flexShrink:0}}><circle cx="7" cy="7" r="5.5" stroke="currentColor" strokeWidth="1.3"/><path d="M4.5 7.5a2.5 2.5 0 015 0" stroke="currentColor" strokeWidth="1.1"/><circle cx="5.5" cy="5" r="1" fill="currentColor"/><circle cx="8.5" cy="5" r="1" fill="currentColor"/></svg>
        Configuración
      </div>
    </div>
    ...
    <div className="sb-user" onClick={()=>{setEditNombre(profile?.nombre||'Equipo Pinta');setShowSettings(true);}} style={{cursor:'pointer'}}>
      <div className="sb-av">{(profile?.nombre||'EA').slice(0,2).toUpperCase()}</div>
      <div className="sb-un">
        <div className="sb-un-name" style={{color:'white'}}>{profile?.nombre||'Equipo Pinta'}</div>
  ...
  <div className="mn">
    {page==='overview'&&<div className="overview-content" style={{padding:'1.5rem'}}>
      <div className="overview-header" style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:16,marginBottom:'1.5rem'}}>
        <div>
        ...
        <div className="overview-actions" style={{display:'flex',alignItems:'center',gap:8}}>
        ...
        {[
          ...
          <div key={label} className="kc" style={{display:'flex',flexDirection:'row',alignItems:'center',gap:14}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:8,flex:1}}>
              ...
              <OverviewSpark data={spark} color={sparkColor||color}/>
            </div>
          </div>
        ))}
      </div>
      <div className="overview-main" style={{display:'flex',gap:16,alignItems:'flex-start'}}>
        {/* Columna izquierda: filtros + clientes */}
        <div style={{flex:1,minWidth:0}}>
          <div className="overview-filters" style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'1rem',gap:8}}>
            <div className="overview-search" style={{width:220,position:'relative'}}>
              ...
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar cliente..." style={{width:'100%',padding:'7px 12px 7px 30px',borderRadius:8,border:'.5px solid var(--bm)',background:'var(--s)',color:'var(--t)',fontWeight:600,fontSize:12}}/>
            </div>
            <div className="overview-filter-actions" style={{display:'flex',alignItems:'center',gap:24}}>
              <select value={filterEstado} onChange={e=>{setFilterEstado(e.target.value);setPagina(1);}} style={{padding:'8px 16px',borderRadius:8,border:'.5px solid var(--bm)',background:'var(--s)',color:'var(--t)',fontSize:12,cursor:'pointer',fontWeight:600,textAlign:'left'}}>
                <option value="todos">Todos los estados</option>
                <option value="activo">Activos</option>
                <option value="revisar">En revisión</option>
                <option value="pausado">Pausados</option>
              </select>
              <select value={sortBy} onChange={e=>{setSortBy(e.target.value);setPagina(1);}} style={{padding:'9px 20px',borderRadius:8,border:'.5px solid var(--bm)',background:'var(--s)',color:'var(--t)',fontSize:12,cursor:'pointer',fontWeight:600,textAlign:'left'}}>
              ...
              <div style={{display:'flex',border:'.5px solid var(--bm)',borderRadius:8,overflow:'hidden'}}>
                <button onClick={()=>setViewMode('grid')} style={{padding:'8px 14px',background:viewMode==='grid'?'var(--t)':'var(--s)',color:viewMode==='grid'?'#fff':'var(--m)',border:'none',cursor:'pointer',display:'flex',alignItems:'center',gap:6,fontSize:12,fontWeight:700}}>
                ...
                <button onClick={()=>setViewMode('list')} style={{padding:'8px 14px',background:viewMode==='list'?'var(--t)':'var(--s)',color:viewMode==='list'?'#fff':'var(--m)',border:'none',borderLeft:'.5px solid var(--bm)',cursor:'pointer',display:'flex',alignItems:'center',gap:6,fontSize:12,fontWeight:700}}>
        ...
        <div style={{width:260,flexShrink:0,display:'flex',flexDirection:'column',gap:12}}>
          ...
          <div style={{background:'var(--s)',border:'.5px solid var(--b)',borderRadius:14,padding:'1rem'}}>
            ...
            {[
              ...
              return(
                <div key={nombre} style={{display:'flex',gap:8,padding:'8px 10px',borderRadius:8,background:'var(--bg)',border:'.5px solid var(--b)',marginBottom:6}}>
    ...
    {page==='client'&&current&&<div className="client-page-content" style={{padding:'1.5rem'}}>
      <div className="client-header" style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:16,marginBottom:'1.5rem'}}>
        ...
        <div className="client-controls" style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:8,flexShrink:0}}>
      ...
      <ClientDashboard client={current} dateFrom={dateFrom} dateTo={dateTo} activeCh={activeCh} onActiveCh={setActiveCh} />
    </div>}
  </div>
</div>
...
{showSettings&&<div className="mb" onClick={e=>e.target===e.currentTarget&&setShowSettings(false)}>
  <div className="mo" style={{background:'#1a1a18',border:'1px solid rgba(255,255,255,.08)',maxWidth:400}}>
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'1.25rem'}}>
      <span style={{fontSize:16,fontWeight:700,color:'#fff'}}>Configuración</span>
      <button onClick={()=>setShowSettings(false)} style={{background:'none',border:'none',color:'rgba(255,255,255,.4)',fontSize:22,cursor:'pointer',lineHeight:1,padding:'0 4px'}}>×</button>
    </div>
    <div style={{marginBottom:14}}>
      <label style={{display:'block',fontSize:11,fontWeight:600,color:'rgba(255,255,255,.35)',textTransform:'uppercase',letterSpacing:'.07em',marginBottom:7}}>Nombre del equipo</label>
      <input value={editNombre} onChange={e=>setEditNombre(e.target.value)}
        style={{width:'100%',padding:'10px 12px',borderRadius:8,border:'1px solid rgba(255,255,255,.1)',background:'rgba(255,255,255,.04)',color:'#fff',fontSize:13,outline:'none',fontFamily:'inherit'}}/>
    </div>
    <div style={{display:'flex',gap:8,justifyContent:'flex-end',marginBottom:'1.25rem'}}>
      <button onClick={()=>setShowSettings(false)} style={{padding:'9px 16px',borderRadius:8,border:'1px solid rgba(255,255,255,.1)',background:'transparent',color:'rgba(255,255,255,.5)',fontSize:13,cursor:'pointer'}}>Cancelar</button>
      <button disabled={savingName} onClick={async()=>{setSavingName(true);await updateProfile({nombre:editNombre.trim()});setSavingName(false);setShowSettings(false);}}
