...
{/* TABS */}
                    ...
                    <div style={{ background: '#fff', border: '.5px solid rgba(0,0,0,.08)', borderRadius: 12, padding: '14px 16px', position: 'relative' }}>
                      ...
                      {ig ? (
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 8 }}>
                          {[['Seguidores', (ig.totals?.follows_and_unfollows >= 0 ? '+' : '') + fmt(ig.totals?.follows_and_unfollows)],['Alcance', fmt(ig.totals?.reach)],['Interacciones', fmt(ig.totals?.total_interactions)],['Visitas perfil', fmt(ig.totals?.profile_views)]].map(([l,v]) => (
        ...
        {activeTab === 'instagram' && (
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,minmax(0,1fr))', gap: 10, marginBottom: 16 }}>
              <KpiCard id="ig_followers" label="Seguidores nuevos" val={(ig.totals?.follows_and_unfollows >= 0 ? '+' : '') + fmt(ig.totals?.follows_and_unfollows)} sub={`total cuenta: ${fmt(ig.account?.followers_count)}`} color="#e1306c" defViz="number" dailyData={[]} vizTypes={vizTypes} setViz={setViz} openMenu={openMenu} setOpenMenu={setOpenMenu} />
