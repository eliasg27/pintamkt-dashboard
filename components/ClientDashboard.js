...
function KPI({ label, val, sub, delta, invertDelta }) {
  ...
  return (
    <div style={{ background: '#fff', border: '.5px solid rgba(0,0,0,.09)', borderRadius: 12, padding: '1rem 1.1rem', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ fontSize: 10, color: 'var(--m)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '.06em', fontWeight: 600 }}>{label}</div>
      <div style={{ fontSize: 24, fontWeight: 700, letterSpacing: '-.02em' }}>{val}</div>
      <div style={{ fontSize: 11, color: 'var(--m)', marginTop: 4, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span>{sub}</span>
        {delta != null && (
          <span style={{
...
function KpiCard({ id, label, val, sub, delta, invertDelta, color, defViz, dailyData, vizTypes, setViz, openMenu, setOpenMenu }) {
  ...
  return (
    <div style={{ background: 'var(--s)', border: '.5px solid var(--b)', borderRadius: 12, padding: '14px 16px', position: 'relative', overflow: 'visible' }}>
...
export default function ClientDashboard({ client: c, dateFrom: df, dateTo: dt }) {
  ...
  const Spinner = () => (
    <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--m)', fontSize: 13 }}>
      <div style={{ width: 20, height: 20, border: '2px solid var(--b)', borderTopColor: '#1D9E75', borderRadius: '50%', animation: 'spin .7s linear infinite', margin: '0 auto 12px' }} />
      Cargando datos...
    </div>
  );
  ...
  return (
    <div onClick={() => setOpenMenu(null)} style={{ fontFamily: "'DM Sans', -apple-system, sans-serif", background: 'var(--bg)', minHeight: '100%', padding: '1.5rem' }}>
      ...
      {/* TOP CARDS */}
      {(()=>{
        const seed=(c.nombre?.length||0)%10;
        ...
        return(
          <div style={{display:'flex',gap:12,marginBottom:'1.5rem'}}>
            ...
            {/* Card 1: Salud de la cuenta */}
            <div style={{background:'var(--s)',border:'.5px solid var(--b)',borderRadius:14,padding:'1.2rem 1.2rem .9rem',width:185,flexShrink:0,position:'relative',overflow:'hidden',display:'flex',flexDirection:'column',justifyContent:'space-between',minHeight:130}}>
              <div>
                <div style={{fontSize:10,fontWeight:600,color:'var(--m)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:10,display:'flex',alignItems:'center',gap:4}}>
                  Salud de la cuenta
              ...
              <div style={{marginTop:14}}>
                <div style={{height:5,background:'var(--b)',borderRadius:3,overflow:'hidden'}}>
                  <div style={{height:'100%',width:health+'%',background:hc,borderRadius:3}}/>
                </div>
              </div>
            </div>
      ...
      {(()=>{
        ...
        const Donut=({spend})=>{
          ...
          const budget=spend*1.25,pct=Math.round((spend/budget)*100);
          const r=18,circ=2*Math.PI*r,dash=(pct/100)*circ;
          return(
            <div style={{display:'flex',alignItems:'center',gap:12,marginTop:12}}>
              <svg width="56" height="56" viewBox="0 0 56 56" style={{flexShrink:0}}>
                <circle cx="28" cy="28" r={r} fill="none" stroke="var(--b)" strokeWidth="6"/>
                <circle cx="28" cy="28" r={r} fill="none" stroke="#f59e0b" strokeWidth="6"
                  strokeDasharray={`${dash} ${circ}`} transform="rotate(-90 28 28)" strokeLinecap="round"/>
                <text x="28" y="32" textAnchor="middle" fontSize="10" fontWeight="700" fill="var(--t)">{pct}%</text>
              </svg>
              <div>
                <div style={{fontSize:10,color:'var(--m)',marginBottom:3}}>del presupuesto</div>
                <div style={{fontSize:12,fontWeight:700,color:'var(--t)'}}>{fm(spend)} / {fm(budget)}</div>
              </div>
            </div>
          );
        };
        const dR=md?.daily?.map(d=>parseInt(d.reach||0))||[];
        const dC=md?.daily?.map(d=>parseInt(d.clicks||0))||[];
        const dT=md?.daily?.map(d=>parseFloat(d.ctr||0))||[];
        const cards=[
          {label:'Alcance',val:fmt(t.reach)||'—',sub:'personas',delta:dl.reach,inv:false,color:'#1D9E75',chart:<Spark data={dR} color="#1D9E75"/>},
          {label:'Clics',val:fmt(t.clicks)||'—',sub:'en anuncios',delta:dl.clicks,inv:false,color:'#2563eb',chart:<Bars data={dC} color="#2563eb"/>},
...
{/* TABS */}
        ...
        {activeTab === 'resumen' && (
            <div onClick={() => setOpenMenu(null)}>
              ...
              {(mods.meta_resumen || mods.meta_rendimiento || mods.meta_resultados || mods.meta_campanas) && c.meta_ad_account_id && (
                <div style={{ marginBottom: 16 }}>
                  <div className="sect-hdr" style={{ marginBottom: 10 }}>
                    ...
                    {metaLoading && <div style={{ width: 12, height: 12, border: '2px solid var(--b)', borderTopColor: '#1D9E75', borderRadius: '50%', animation: 'spin .7s linear infinite', marginLeft: 8 }} />}
                  </div>
                  ...
                  ) : <div style={{ fontSize: 12, color: 'var(--m)', padding: '1rem' }}>Sin datos para el período.</div>}
                </div>
              )}
              ...
              {(mods.instagram_organico || mods.facebook_organico) && (
                <div style={{ display: 'grid', gridTemplateColumns: mods.instagram_organico && mods.facebook_organico ? '1fr 1fr' : '1fr', gap: 10, marginBottom: 16 }}>
                  {mods.instagram_organico && c.ig_account_id && (
                    <div style={{ background: 'var(--s)', border: '.5px solid var(--b)', borderRadius: 12, padding: '14px 16px', position: 'relative' }}>
                      ...
                      <div className="sect-hdr">
                        ...
                        {igLoading && <div style={{ width: 12, height: 12, border: '2px solid var(--b)', borderTopColor: '#e1306c', borderRadius: '50%', animation: 'spin .7s linear infinite', marginLeft: 8 }} />}
                      </div>
                      {ig ? (
                  ...
                  {mods.facebook_organico && c.fb_page_id && (
                    <div style={{ background: 'var(--s)', border: '.5px solid var(--b)', borderRadius: 12, padding: '14px 16px', position: 'relative' }}>
                      ...
                      <div className="sect-hdr">
                        ...
                        {fbLoading && <div style={{ width: 12, height: 12, border: '2px solid var(--b)', borderTopColor: '#1877f2', borderRadius: '50%', animation: 'spin .7s linear infinite', marginLeft: 8 }} />}
                      </div>
                      {fb ? (
              ...
              {/* GA4 */}
              {mods.ga4 && (
                <div style={{ background: 'var(--s)', border: '.5px solid var(--b)', borderRadius: 12, padding: '14px 16px', marginBottom: 16, position: 'relative' }}>
                  ...
                  <div className="sect-hdr">
                    ...
                    {ga4Loading && <div style={{ width: 12, height: 12, border: '2px solid var(--b)', borderTopColor: '#4285f4', borderRadius: '50%', animation: 'spin .7s linear infinite', marginLeft: 8 }} />}
                  </div>
                  {ga4 ? (
              ...
              {mods.woocommerce && (
                <div style={{ background: 'var(--s)', border: '.5px solid var(--b)', borderRadius: 12, padding: '14px 16px', marginBottom: 16, position: 'relative' }}>
                  ...
                  <div className="sect-hdr">
                    ...
                    {wooLoading && <div style={{ width: 12, height: 12, border: '2px solid var(--b)', borderTopColor: '#7f54b3', borderRadius: '50%', animation: 'spin .7s linear infinite', marginLeft: 8 }} />}
                  </div>
                  {woo ? (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,minmax(0,1fr))', gap: 10 }}>
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,minmax(0,1fr))', gap: 10, marginBottom: 16 }}>
              {[
                ...
                { label: 'Frecuencia', val: t.frequency ? t.frequency.toFixed(2) : '—', sub: 'veces/persona', delta: null, inv: false, color: 'var(--m)' },
              ].map(k => {
                const good = k.delta == null ? null : (k.inv ? k.delta <= 0 : k.delta >= 0);
                return (
                  <div key={k.label} style={{ background: 'var(--s)', border: '.5px solid var(--b)', borderRadius: 12, padding: '14px 16px', position: 'relative' }}>
            ...
            {/* GRÁFICO DIARIO */}
            <div style={{ background: 'var(--s)', border: '.5px solid var(--b)', borderRadius: 12, padding: '1rem 1.1rem', marginBottom: 16 }}>
              ...
              <div style={{ background: 'var(--s)', border: '.5px solid var(--b)', borderRadius: 12, overflow: 'hidden', marginBottom: 16 }}>
                {camps.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '2rem' }}>
                    <div style={{ color: 'var(--m)', fontSize: 13, marginBottom: 12 }}>No se encontraron campañas en este período.</div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'DM Sans,sans-serif' }}>
                    ...
                    <tbody>
                      {camps.map((camp, i) => {
                        ...
                        return (
                          <tr key={i} style={{ borderBottom: '.5px solid var(--b)', cursor: 'pointer' }} onClick={() => {
                            ...
                            <td style={{ padding: '12px 14px', fontSize: 13, color: 'var(--m)' }}>{fmt(parseInt(camp.impressions || 0))}</td>
                            <td style={{ padding: '12px 14px', fontSize: 13, color: 'var(--m)' }}>{fmt(parseInt(camp.clicks || 0))}</td>
                      ...
                      {(drillData?.adsets || []).map((adset, i) => {
                        ...
                        return (
                          <tr key={i} style={{ borderBottom: '.5px solid var(--b)', cursor: 'pointer' }} onClick={() => {
                            ...
                            <td style={{ padding: '12px 14px', fontSize: 13, color: 'var(--m)' }}>{fmt(parseInt(ins.impressions || 0))}</td>
                            <td style={{ padding: '12px 14px', fontSize: 13, color: 'var(--m)' }}>{fmt(parseInt(ins.clicks || 0))}</td>
                            ...
                            <td style={{ padding: '12px 14px', fontSize: 12, color: 'var(--m)' }}>{budget}</td>
                            <td style={{ padding: '12px 14px', color: 'var(--m)', fontSize: 12 }}>→</td>
                          </tr>
                        );
                      })}
                      {!drillLoading && (drillData?.adsets || []).length === 0 && (
                        <tr><td colSpan={8} style={{ padding: '2rem', textAlign: 'center', color: 'var(--m)', fontSize: 13 }}>Sin ad sets encontrados.</td></tr>
                      )}
                    </tbody>
                  </table>
                )}
            ...
            {drillAdset && (
              <div style={{ marginBottom: 16 }}>
                {drillLoading ? <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--m)' }}>Cargando anuncios...</div> : (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,minmax(0,1fr))', gap: 12 }}>
                    {(drillData?.ads || []).map((ad, i) => {
                      ...
                      return (
                        <div key={i} style={{ background: 'var(--s)', border: '.5px solid var(--b)', borderRadius: 12, overflow: 'hidden' }}>
                          ...
                          {!img && <div style={{ width: '100%', aspectRatio: '1.91', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24 }}>🎨</div>}
                    ...
                    {!drillLoading && (drillData?.ads || []).length === 0 && (
                      <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '2rem', color: 'var(--m)', fontSize: 13 }}>Sin anuncios encontrados.</div>
                    )}
                  </div>
                )}
              </div>
              ...
              <>
                ...
                <div style={{ background: 'var(--s)', border: '.5px solid var(--b)', borderRadius: 12, overflow: 'hidden', marginBottom: 16 }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'DM Sans,sans-serif' }}>
                    ...
                    <tbody>
                      {fb.posts.map((post, i) => (
                        <tr key={i} style={{ borderBottom: '.5px solid var(--b)' }}>
                          ...
                          <td style={{ padding: '12px 14px', fontSize: 13, color: 'var(--m)' }}>{post.likes || 0}</td>
                          <td style={{ padding: '12px 14px', fontSize: 13, color: 'var(--m)' }}>{post.comments || 0}</td>
                          <td style={{ padding: '12px 14px', fontSize: 13, color: 'var(--m)' }}>{post.shares || 0}</td>
                          <td style={{ padding: '12px 14px', fontSize: 12, color: 'var(--m)' }}>{post.created_time ? new Date(post.created_time).toLocaleDateString('es-AR') : '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  {ig.posts.slice(0,12).map((post, i) => (
                    <div key={i} style={{ background: 'var(--s)', border: '.5px solid var(--b)', borderRadius: 12, overflow: 'hidden' }}>
                      {post.thumbnail_url || post.media_url ? (
                        ...
                        <div style={{ width: '100%', aspectRatio: '1', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24 }}>📸</div>
                      )}
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
              {ga4.sources?.length > 0 && (
                <div style={{ background: 'var(--s)', border: '.5px solid var(--b)', borderRadius: 12, padding: '14px 16px' }}>
